...
const ResumeBuilderContent = () => {
  ...
  const handlePasteJson = () => {
    const input = window.prompt("Paste the JSON generated by ChatGPT here:");
    if (!input) return;
    try {
      const parsed = JSON.parse(input);
      if (parsed.personalInfo || parsed.summary || parsed.workExperience) {
        setResumeData(parsed);
        setHasUnsavedChanges(true);
        localStorage.setItem(STORAGE_KEY, JSON.stringify(parsed));
        showNotification('JSON imported successfully! Make sure to verify the preview.', 'success');
      } else {
        throw new Error("Invalid structure");
      }
    } catch (e) {
      showNotification('Invalid JSON pasted. Please ensure it matches the correct structure.', 'danger');
    }
  };
              ...
              {activeTab === 'resume' && (
                <>
                  <Button
                    ...
                    title="Paste AI-generated JSON directly to build resume"
                  >
